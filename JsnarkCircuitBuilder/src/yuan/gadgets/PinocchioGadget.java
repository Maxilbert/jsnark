 ...
 * Modifier: Yuan Lu <yl768@njit.edu>
 * Modification: distinguish public input wires and prover witness wires
 *******************************************************************************/
...
public class PinocchioGadget extends Gadget {
	...
	public PinocchioGadget(Wire[] inputWires, Wire[] proverWitnessWires, String pathToArithFile, String... desc) {
		super(desc);
		this.inputWires = inputWires;
		this.proverWitnessWires = proverWitnessWires;
		try {
	...
	private void buildCircuit(String path) throws FileNotFoundException {
		...
		while (scanner.hasNext()) {
			...
			if (line.equals("")) {
				...
				if (inputWires != null && inputCount < inputWires.length) {
				...
				if (proverWitnessInputCount < proverWitnessWires.length) {
					wireMapping[wireIndex] = proverWitnessWires[proverWitnessInputCount];
				} else {
					// the last input wire is assumed to be the one wire
					wireMapping[wireIndex] = generator.getOneWire();
				}
				proverWitnessInputCount++;
			} else {
